...
class CocoDetection(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        if 'im_name' in coco.loadImgs(img_id)[0]:
            path = coco.loadImgs(img_id)[0]['im_name']
            path = os.path.join(path.split('_')[0] + '/'+ path.split('_')[0] + '_' +
                            path.split('_')[1] + '_' + path.split('_')[2] + '_' + path.split('_')[3])
        else:
