...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor,support_imgs, true_atten,mode='training'):
        ...
        hs, pred_atten, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(srcs, masks, pos,support_embedd,orignal_shape,self.num_queries,true_atten,mode,query_embeds)
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],'pred_atten': pred_atten}
        if self.aux_loss:
...
class PostProcess_caltech(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        #     class_idx = torch.arange(0,80,device=scores.device)
        #     idx = self.non_max_suppression(class_idx, result['labels'], result['boxes'], result['scores'], 0.1)
...
def build(args):
    ...
    if args.dataset_file == "coco_panoptic":
        num_classes = 250
    if args.dataset_file == 'cityperson':
        num_classes = 91
